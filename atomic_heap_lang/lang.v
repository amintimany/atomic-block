...
From atomic_block Require Export atomic_ectx_language atomic_ectxi_language.
From iris.algebra Require Export ofe.
From stdpp Require Export strings.
From stdpp Require Import gmap.
Set Default Proof Using "Type".

(** atomic_heap_lang. A version of the language heap_lang provided
with iris featuring atomic blocks. The following is the description of
heap_lang from the Iris development.

A fairly simple language used for common Iris examples.

- This is a right-to-left evaluated language, like CakeML and OCaml.
  The reason for this is that it makes curried functions usable: Given
  a WP for [f a b], we know that any effects [f] might have to not
  matter until after *both* [a] and [b] are evaluated.  With
  left-to-right evaluation, that triple is basically useless unless
  the user let-expands [b].

- For prophecy variables, we annotate the reduction steps with an
  "observation" and tweak adequacy such that WP knows all future
  observations.  There is another possible choice: Use
  non-deterministic choice when creating a prophecy variable
  ([NewProph]), and when resolving it ([ResolveProph]) make the
  program diverge unless the variable matches.  That, however,
  requires an erasure proof that this endless loop does not make
  ...
  refine (
   fix go (e1 e2 : expr) {struct e1} : Decision (e1 = e2) :=
     ...
     | ResolveProph e1 e2, ResolveProph e1' e2' =>
        cast_if_and (decide (e1 = e1')) (decide (e2 = e2'))
     | AtomicBlock e, AtomicBlock e' => cast_if (decide (e = e'))
     | _, _ => right _
     end
...
Fixpoint subst (x : string) (v : val) (e : expr)  : expr :=
  ...
  | AtomicBlock e => AtomicBlock (subst x v e)
  end.
...
Definition atomic_block_of (e : expr) :=
  match e with
  | AtomicBlock ab => Some ab
  | _ => None
  end.

Lemma atomic_block_head_stuck e1 σ1 κ e2 σ2 efs :
  head_step e1 σ1 κ e2 σ2 efs → atomic_block_of e1 = None.
Proof. destruct e1; auto; inversion 1. Qed.
Lemma atomic_block_not_val e ab : atomic_block_of e = Some ab → to_val e = None.
Proof. by destruct e. Qed.
Lemma atomic_block_no_ectx e ae :
  atomic_block_of e = Some ae → ∀ (Ki : ectx_item) (e' : expr), e ≠ fill_item Ki e'.
Proof.
  destruct e; inversion 1; simplify_eq.
  intros Ki e'; by destruct Ki; simpl.
Qed.

Lemma heap_lang_mixin :
  AtomicEctxiLanguageMixin of_val to_val atomic_block_of fill_item head_step.
Proof.
  split; apply _ || eauto using to_of_val, of_to_val, val_head_stuck,
    fill_item_val, fill_item_no_val_inj, head_ctx_step_val,
    atomic_block_head_stuck, atomic_block_not_val, atomic_block_no_ectx.
Qed.
End heap_lang.

(** Language *)
Canonical Structure heap_ectxi_lang := AtomicEctxiLanguage heap_lang.heap_lang_mixin.
Canonical Structure heap_ectx_lang := AtomicEctxLanguageOfEctxi heap_ectxi_lang.
Canonical Structure heap_lang := AtomicLanguageOfEctx heap_ectx_lang.
