...
\ruleref{Basic-WP-Atomic-Block} and \ruleref{WP-Atomic-Block}
...
{\later \abwpre{\expr}{\pred} \and
  ...
  \FromCoq{\prophids(\observation) = \prophids(\observation')} \ast
    \left((\forall \ploc.\; \FromCoq{\observation(\ploc) = \observation'(\ploc)}) \wand \predB(\val)\right)}
{\wpre{\AtomicBlock{\expr}}[\mask]{\var.\; \predB(\val)}}
\end{mathparpagebreakable}
Where $\prophids(\observation)$ is the set of prophecy identifiers
...
\begin{mathparpagebreakable}
  \inferH{later-basic-update}{}{\later \upd \prop \provesIff \later \upd \prop}
\end{mathparpagebreakable}
...
to derive the rule \ruleref{Basic-WP-Atomic-Block} above. We would be
